...
export default function AdminAboutPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        console.log("[v0] Loading about page data...")
        setIsLoading(true)
        ...
        console.log("[v0] Loaded config:", config)
        ...
        if (translationsRes.ok) {
          ...
          console.log("[v0] Loaded translations:", translations)
        ...
        console.log("[v0] Loading team members from team_translations table...")
        const members = await getAllTeamMembers()
        console.log("[v0] Loaded team members:", members)
        setTeamMembers(members)
      } catch (error) {
        console.error("[v0] Error loading about page data:", error)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log("[v0] Saving statistics:", statTitles)
      const statsPayload = {
        statistics: {
      ...
      if (aboutImageUrl) {
        console.log("[v0] Saving about image:", aboutImageUrl)
        ...
        console.log("[v0] About image saved successfully")
      }

      // Step 3: Save team data to team_translations table
      console.log("[v0] Saving team data...")
      for (const member of teamMembers) {
        ...
        for (const lang of ["en", "ru", "kz"] as const) {
          if (member.name[lang]) {
            ...
            if (!nameRes.ok) {
              console.error(`[v0] Failed to save name for ${slug} (${lang})`)
            }
          }
        }
        ...
        for (const lang of ["en", "ru", "kz"] as const) {
          if (member.role[lang]) {
            ...
            if (!roleRes.ok) {
              console.error(`[v0] Failed to save role for ${slug} (${lang})`)
            }
          }
        }
        ...
        for (const lang of ["en", "ru", "kz"] as const) {
          if (member.bioLeft[lang]) {
            ...
            if (!bioLeftRes.ok) {
              console.error(`[v0] Failed to save bio_left for ${slug} (${lang})`)
            }
          }
        }
        ...
        for (const lang of ["en", "ru", "kz"] as const) {
          if (member.bioRight[lang]) {
            ...
            if (!bioRightRes.ok) {
              console.error(`[v0] Failed to save bio_right for ${slug} (${lang})`)
            }
          }
        }
        ...
        if (member.photo) {
          ...
          if (!photoRes.ok) {
            console.error(`[v0] Failed to save photo for ${slug}`)
          }
        }
      }
      console.log("[v0] Team data saved successfully")

      // Step 4: Save other translations to translations table
      console.log("[v0] Saving other translations...")
      const updatedI18n = {
      ...
      console.log("[v0] Translations saved successfully")
      ...
      console.error("[v0] Error saving about page data:", e)
