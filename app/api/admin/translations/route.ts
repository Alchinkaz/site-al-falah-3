...
export async function POST(request: Request) {
  try {
    ...
    console.log("[v0] POST /api/admin/translations - Received body keys:", Object.keys(body))
    ...
    for (const [key, val] of Object.entries(updates as Record<string, any>)) {
      console.log(`[v0] Processing key: ${key}`)
...
async function processValue(supabaseAdmin: any, key: string, val: any, langs: Array<"en" | "ru" | "kz">) {
  ...
  if (Array.isArray(val)) {
    ...
    console.log(`[v0] Saving array for key ${key}:`, jsonString.substring(0, 100))
    for (const lang of langs) {
  ...
  if (typeof val === "object") {
    ...
    if (hasLangKeys) {
      console.log(`[v0] Saving language object for key ${key}`)
      ...
      console.log(`[v0] Saving nested object for key ${key}`)
...
async function upsertTranslation(supabaseAdmin: any, key: string, lang: "en" | "ru" | "kz", value: any) {
  ...
  if (error) {
    console.error(`[v0] Error upserting translation for ${key}.${lang}:`, error)
    throw error
  }
}
