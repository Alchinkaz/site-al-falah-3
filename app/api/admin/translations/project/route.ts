...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[v0] Error fetching project translations:", error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    console.log("[v0] Loaded project translations for", projectId, ":", data?.length, "rows")
    ...
    console.error("[v0] Project translations GET error:", e)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Saving project translations for", projectId)
    ...
    if (title) {
      for (const lang of ["en", "ru", "kz"] as const) {
        ...
        if (error) {
          console.error(`[v0] Error saving title translation (${lang}):`, error)
        }
      }
    }
    ...
    if (badges) {
      for (const lang of ["en", "ru", "kz"] as const) {
        ...
        if (error) {
          console.error(`[v0] Error saving badges translation (${lang}):`, error)
        }
      }
    }
    ...
    if (sections) {
      for (const lang of ["en", "ru", "kz"] as const) {
        ...
        if (error) {
          console.error(`[v0] Error saving sections translation (${lang}):`, error)
        }
      }
    }

    console.log("[v0] Successfully saved project translations")

    return NextResponse.json({ success: true })
  } catch (e: any) {
    console.error("[v0] Project translations POST error:", e)
    return NextResponse.json({ error: e?.message || "Internal server error" }, { status: 500 })
  }
}
