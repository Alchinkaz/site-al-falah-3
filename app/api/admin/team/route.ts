...
export async function POST(request: Request) {
  try {
    ...
    console.log("[v0] POST /api/admin/team - Received body:", body)

    if (body.slug && body.type && body.language && body.value !== undefined) {
      const { slug, type, language, value } = body
      console.log(`[v0] Updating ${type} for ${slug} (${language}):`, value)
      ...
      console.log(`[v0] Successfully updated ${type} for ${slug} (${language})`)
    ...
    if (body.slug && body.photo !== undefined) {
      ...
      console.log(`[v0] Updating photo for ${slug}:`, photo)
      ...
      console.log(`[v0] Successfully updated photo for ${slug}`)
    ...
    if (teamPhotos) {
      for (const [slug, photo] of Object.entries(teamPhotos as Record<string, string>)) {
        console.log(`[v0] Saving photo for ${slug}:`, photo)
    ...
    if (teamNames) {
      for (const [slug, names] of Object.entries(teamNames as Record<string, any>)) {
        console.log(`[v0] Saving names for ${slug}`)
    ...
    if (teamI18n) {
      for (const [slug, data] of Object.entries(teamI18n as Record<string, any>)) {
        console.log(`[v0] Saving i18n for ${slug}`)
    ...
    console.log("[v0] All team data saved successfully to team_translations table")
    ...
    console.error("[v0] Team API error:", e)
