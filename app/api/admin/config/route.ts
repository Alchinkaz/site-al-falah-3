...
export async function POST(request: Request) {
  try {
    ...
    console.log("[v0] POST /api/admin/config - received keys:", Object.keys(payload))
    ...
    const upsertPromises = Object.entries(payload).map(async ([key, value]) => {
      console.log(`[v0] Upserting key: ${key}, value:`, JSON.stringify(value).substring(0, 200))
      ...
      if (result.error) {
        console.error(`[v0] ❌ ERROR upserting key ${key}:`, result.error)
        throw new Error(`Failed to save ${key}: ${result.error.message}`)
      } else {
        console.log(`[v0] ✅ Successfully upserted key: ${key}`)
        ...
        if (verifyError) {
          console.error(`[v0] ⚠️ Could not verify save for key ${key}:`, verifyError)
        } else {
          console.log(`[v0] ✓ Verified key ${key} in database:`, JSON.stringify(verifyData.value).substring(0, 100))
        }
      }
    ...
    if (errors.length > 0) {
      console.error(
        "[v0] Errors upserting data:",
        errors.map((e) => e.error),
      )
      return NextResponse.json({ error: errors[0].error?.message || "Failed to save data" }, { status: 500 })
    }

    console.log("[v0] ✅ Successfully saved", Object.keys(payload).length, "config keys")
    ...
    console.error("[v0] POST /api/admin/config error:", e)
    return NextResponse.json({ error: e?.message || "Internal error" }, { status: 500 })
  }
}
