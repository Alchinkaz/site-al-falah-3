...
export async function getTeamMemberData(slug: string): Promise<TeamMemberData | null> {
  try {
    ...
    if (error) {
      console.error("[v0] Error fetching team member data:", error)
      return null
    }

    if (!data || data.length === 0) {
      console.log("[v0] No team data found for slug:", slug)
      return null
    }
    ...
    console.log("[v0] Loaded team member data from Supabase:", result)
    return result
  } catch (error) {
    console.error("[v0] Exception fetching team member data:", error)
    return null
  }
}
...
export async function getAllTeamMembers(): Promise<TeamMemberData[]> {
  try {
    ...
    if (!members || members.length === 0) {
      console.log("[v0] No team members found in database")
      return []
    }
    ...
    if (error) {
      console.error("[v0] Error fetching all team translations:", error)
      return []
    }
    ...
    console.log("[v0] Loaded all team members from Supabase:", result.length)
    return result
  } catch (error) {
    console.error("[v0] Exception fetching all team members:", error)
    return []
  }
}
