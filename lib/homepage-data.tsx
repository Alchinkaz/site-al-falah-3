...
export async function getHomepageData(bustCache = false): Promise<HomepageData> {
  try {
    console.log("[v0] Fetching homepage data from Supabase...")
    const config = await HomepageService.getHomepageData()
    console.log("[v0] Received homepage data:", JSON.stringify(config).substring(0, 500))
    ...
    console.log("[v0] Extracted statistics:", { stat1Title, stat2Title, stat3Title })
    ...
    console.log("[v0] Extracted footer data:", { footerEmail, footerCopyright })
    ...
    console.error("[v0] Error fetching homepage data from Supabase:", error)
