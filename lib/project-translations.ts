...
export async function getProjectTranslations(projectId: string): Promise<ProjectTranslations | null> {
  try {
    ...
    if (error) {
      console.error(`[v0] Error fetching translations for ${projectId}:`, error)
      return null
    }
...
export async function getAllProjectTranslations(projectIds: string[]): Promise<Record<string, ProjectTranslations>> {
  try {
    ...
    if (typeof window === "undefined") {
      ...
      console.log("[v0] Fetching translations for projects:", projectIds)
      const response = await fetch(`/api/admin/translations/projects?ids=${projectIds.join(",")}`)

      if (!response.ok) {
        console.error("[v0] Failed to fetch project translations:", response.status, response.statusText)
        return {}
      }

      const data = await response.json()
      console.log("[v0] Received project translations:", data)
      return data.translations || {}
    }
  } catch (error) {
    console.error("[v0] Error fetching all project translations:", error)
    return {}
  }
}
