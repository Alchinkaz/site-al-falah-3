...
export function NewsEditForm({ article, onSave, onCancel, hideHeader }: NewsEditFormProps) {
  ...
  useEffect(() => {
    const loadTranslations = async () => {
      ...
      try {
        console.log("[v0] Loading project translations for:", article.id)
        ...
        console.log("[v0] Loaded project translations:", data)
  ...
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (newLang === "en" || newLang === "ru" || newLang === "kz") {
        console.log("[v0] Language switched to:", newLang)
  ...
  const handleSave = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        console.error("[v0] Error saving translations:", err)
      }
    }, 500)
  }
