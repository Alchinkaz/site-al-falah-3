...
export function NewsManagement({ currentUser, formatDate, currentLang }: NewsManagementProps) {
  ...
  const handleSaveArticle = async (articleData: any) => {
    try {
      ...
      if (showAddForm) {
        ...
        console.log("[v0] Creating project with payload:", insertPayload)
        ...
        console.log("[v0] Updating project with payload:", updates)
  ...
  const loadProjectTranslations = async () => {
    try {
      console.log("[v0] Loading translations for", articles.length, "projects")
      ...
      for (const article of articles) {
        try {
          ...
          if (res.ok) {
            ...
            console.log(`[v0] Loaded translations for ${article.id}:`, data)
            translations[article.id] = data
          }
        } catch (err) {
          console.error(`[v0] Error loading translations for ${article.id}:`, err)
        }
      }

      console.log("[v0] All project translations loaded:", translations)
          ...
          {articles.length === 0 ? (
            ...
            <div className="space-y-4">
              {articles.map((article) => {
                ...
                console.log(`[v0] Rendering project ${article.id}:`, { title, description, badges })
