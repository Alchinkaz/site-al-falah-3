...
export default function Navbar({ forceScrolled = false }: { forceScrolled?: boolean } = {}) {
  ...
  return (
    <>
      ...
      {shouldShowScrolled && !mobileMenuOpen && (
        <header
          ...
          <div className="h-full flex items-center">
            <div className="max-w-[22rem] sm:max-w-md md:max-w-3xl lg:max-w-5xl xl:max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 w-full">
              ...
              <div className="mt-12">
                <button
                  onClick={() => setLanguageDropdownOpen(!languageDropdownOpen)}
                  className="flex items-center justify-center w-10 h-10 rounded-lg bg-white/10 hover:bg-white/20 text-white"
                  aria-label="Select language"
                >
                  <Globe className="w-5 h-5" />
                </button>
                {languageDropdownOpen && (
                  <div className="mt-3 bg-white rounded-lg shadow-lg border border-gray-200 py-2 min-w-[120px] z-50">
                    <button
                      onClick={() => handleLanguageChange("kz")}
